"""Serviços de domínio para e-mails transacionais (OTP 2FA e recuperação de senha)."""
...
def enviar_otp_login(user: User, codigo_plain: str, magic_url: str) -> None:
    ...
    subject = "Código de verificação — Biblioteca Quintal das Artes"
    text_body = (
        f"Olá, {nome}.\n\n"
...
def enviar_redefinicao_senha(user: User, reset_url: str, *, minutos: int = 30) -> None:
    ...
    nome = (user.first_name or user.username or "").strip() or "usuário"
    from_email = settings.DEFAULT_FROM_EMAIL
    subject = "Redefinir senha — Biblioteca Quintal das Artes"
    text_body = (
        f"Olá, {nome}.\n\n"
